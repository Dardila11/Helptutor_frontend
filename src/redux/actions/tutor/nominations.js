...
export const addNomination = (data) => (dispatch, getState) =>{
    ...
    Api.postNomination(data,getState).then((res)=>{
        dispatch({
            type: ADD_NOMINATION,
            payload: res.data
        })
...
export const getNomination = (id) => (dispatch, getState) => {
    Api.getNomination(id,getState).then((res)=> {
        dispatch({
            type: GET_NOMINATION,
            payload: res.data
        })
...
export const getNominations = () => (dispatch, getState) => {
    Api.getNominations(getState).then((res) => {
        dispatch({
        ...
        dispatch(launchAlert('Error obteniendo postulaciones', err.response.status))
    })
}

export const updateNomination = (id,data) => (dispatch, getState) => {
    Api.patchNomination(id,data,getState).then((res) => {
        dispatch({
        ...
        dispatch(launchAlert('Error actualizando nominación', err.response.status))
    })
}

export const deleteNomination = (id) => (dispatch, getState) => {
    Api.deleteNomination(id,getState).then((res) => {
        dispatch({
